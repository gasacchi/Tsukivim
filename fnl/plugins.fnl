...
(packer.startup 
  (fn [use]
    ...
    (let [{:config lsp-config} (require :modules.lsp)
          ...
          {:config nvim-startup-config} (require :modules.utils.nvim-startup)
          {:config dashboard-config} (require :modules.ui.dashboard)
    ...
    ;; see modules/utils/nvim-startup.fnl for configure nvim-startup
     (use {1 :henriquehbr/nvim-startup.lua
           :config (nvim-startup-config)})
