...
(local themes (require :modules.telescope.themes))
  ...
  (let [(ok? builtin) (tsv.require-plugin :telescope.builtin)]
    (if ok?
      ...
      :otherwise (tsv.notify.error 
                   builtin 
                   [:fn:picker :modules.telescope.commands]))))
...
(fn git-files []
  ...
  (let [(ok? err) (pcall picker :git_files "  Git files")]
    (when (not ok?)
        (tsv.notify.warn err
                         [:fn:git-files :modules.telescope.commands])
        (find-files))))
...
(fn projects []
  ...
  (let [(ok? telescope) (tsv.require-plugin :telescope)] 
    (if ok? 
      ...
      :otherwise (tsv.notify.error 
                   telescope 
                   [:fn:projects :modules.telescope.commands]))))
